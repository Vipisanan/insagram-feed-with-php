...
class StudioScheduleController extends WP_REST_Controller
{
    ...
    public function register_routes(){
        ...
        register_rest_route($namespace, '/' . 'schedule-all', [
            array(
                'methods' => 'GET',
        ...
        register_rest_route($namespace, '/' . 'schedule', [
            array(
                'methods' => 'GET',
                'callback' => array($this, 'getAllSlotsFromToday'),
                'permission_callback' => array($this, 'get_items_permissions_check'),
                'args' => array(),
            )
        ]);
    ...
    function getAllSlotsFromToday(){
        $scheduleService = new ScheduleService();
        $result = $scheduleService->getAllSlotsFromToday();
        if (empty($result)) {
            return new WP_Error('empty_category', 'there is no slot', array('status' => 404));
